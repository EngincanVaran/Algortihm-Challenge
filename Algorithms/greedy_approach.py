...
def calculate_time_for_vehicle(matrix, vehicle_pos, job_pos):
    if matrix[vehicle_pos][job_pos] == 0:
        return -1
    return matrix[vehicle_pos][job_pos]
        ...
        for job in jobs:
            if job["id"] not in visited_jobs:
                temp[job["id"]-1] = calculate_time_for_vehicle(
                    matrix, vehicle["start_index"], job["location_index"])
...
def greedy(matrix, jobs, vehicles):
